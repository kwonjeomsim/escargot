...
Value ShadowRealmObject::exportGetter(ExecutionState& state, Value thisValue, size_t argc, Value* argv, Optional<Object*> newTarget)
{
    ...
    return getWrappedValue(state, state.context(), val);
}
...
Value ShadowRealmObject::shadowRealmImportValue(ExecutionState& state, String* specifierString, String* exportName, Context* callerRealm, Context* evalRealm)
{
    ...
    // Script::finishLoadingImportedModule(*evalContext, evalRealm, Script::ModuleRequest(specifierString, Platform::ModuleType::ModuleES));
    ...
    ExtendedNativeFunctionObject* onFulfilled = new ExtendedNativeFunctionObjectImpl<1>(state, NativeFunctionInfo(AtomicString(state, exportName), ShadowRealmObject::exportGetter, 1));
    onFulfilled->setInternalSlotAsPointer(0, innerCapability.m_promise);
    ...
    onRejected->setInternalSlotAsPointer(0, innerCapability.m_promise);
